...
Use the probaverse to make a discrete distribution of length of stay:

```{r, fig.width = 4, height = 3}
dst_los <- dst_empirical(ndays, data = los, weights = p)
plot(dst_los)
```
...
We can compute conditional distributions in this way using distplyr's `slice_*()` operators. In this case, we are slicing off the left part of the distribution starting at LOS = 2.

```{r}
dst_los_conditional <- slice_left(dst_los, breakpoint = 2)
plot(dst_los_conditional)
```
...
Consider the joint distribution of _length of stay_ of a ship, and its _gang demand_. 
...
Here's the marginal of LOS. Notice that the distribution of LOS is the same as before!
...
Similarly, the distribution of gang request is:
